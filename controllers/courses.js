...
router.post('/create', isLoggedIn, function (req, res) {
  ...
  Course.findByCourseName(coursename, function (err, doc) {
    if (err) res.send('Some error occured')
    else if (doc[0]) res.redirect('/courses/new')
    else {
...
router.get('/edit', isLoggedIn, function (req, res) {
  ...
  Course.getByCourseId(courseid, function (err, doc) {
    if (err) res.send('Some error occured')
      ...
      if (doc) res.render('courses/edit', { title: 'Edit Course', course: doc })
      else
        res.render('courses/get_courseid_edit', {
          title: 'Get course ID',
          courseid: default_courseid,
...
router.post('/update', isLoggedIn, function (req, res) {
  ...
  Course.update(prevcourseid, course, function (err, doc) {
    if (err) res.render('courses/edit', { title: 'Edit Course', course: doc })
    else res.redirect('../admin/home')
  })
})
