...
router.post('/create', isLoggedIn, function (req, res) {
  ...
  Faculty.getByUserName(username, function (err, doc) {
    ...
    else if (doc) res.redirect('/faculties/new')
    else {
...
router.get('/edit', isLoggedIn, function (req, res) {
  ...
  Faculty.getByUserName(username, function (err, doc) {
    if (err) res.send('Some error occured')
      ...
      else res.redirect('/faculties/get_username_edit')
    }
  })
})
...
router.post('/delete', isLoggedIn, function (req, res) {
  ...
  Faculty.getByUserName(username, function (err, doc) {
    ...
    } else res.redirect('../faculties/get_username_delete')
  })
})

router.get('/assign_form', isLoggedIn, function (req, res) {
  res.render('faculties/assign', {
    title: 'Assign',
    username: default_username,
          ...
          Faculty.getByCourseId(username, course_code, function (err, doc) {
            if (err) res.send('Some error occured')
              ...
              Faculty.assign(username, course_code, function (err, doc) {
                if (err) res.send('Some error occured')
                else if (doc) res.redirect('/admin/home')
              })
            }
          })
        } else res.redirect('/faculties/assign_form')
      })
    } else res.redirect('/faculties/assign_form')
  })
})
  ...
  Faculty.getByUserName(username, function (err, doc) {
    if (err) res.send('Some error occured')
      ...
      Course.getBycourseid(course_code, function (err, doc) {
        ...
        } else res.redirect('/faculties/unassign_form')
      })
    } else res.redirect('/faculties/unassign_form')
  })
})
